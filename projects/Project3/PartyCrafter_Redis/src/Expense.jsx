...
function Expense({ expense, onFetchEventDetail, eventId }) {
    ...
    return (
        <Container>
            <Row>
                <Col md={4}>{expense.description}</Col>
                <Col md={3}>
                    {`${formattedDate.getYear()}-${formattedDate.getMonth()}-${formattedDate.getDay()}`}
                </Col>
                <Col md={3}>${expense.amount_spent}</Col>
                <Col><Button variant="danger" onClick={() => {onFetchDeleteExpense(expense.expense_id)}}><i className="gg-trash"></i></Button></Col>
            </Row>
        </Container>
